...
anova.sh = aov(richness1$Shannon ~ sample_data(physeq1)$Age)
summary(anova.sh)
TukeyHSD(anova.sh)
...
kruskal.test(richness1$Shannon ~ sample_data(physeq1)$Age)
# We can also get a list with the p-values resulted of the Wilcoxon Tests considering each pair of groups:
pairwise.wilcox.test(richness1$Shannon, sample_data(physeq1)$Age, p.adj = "bonf")
...
plot_richness(physeq1, x="Age", measures="Simpson", color = "Age")+
  geom_boxplot(alpha=0.6)+ 
  theme(legend.position="none", axis.text.x=element_text(angle=45,hjust=1,vjust=1,size=12))
...
# If it is normally distributed, we can run anova tests and check if the variable Age impacts the Simpson diversity:
anova.sh = aov(richness2$Simpson ~ sample_data(physeq1)$Age)
summary(anova.sh)
TukeyHSD(anova.sh)
...
# For Non-normally distributed data, we can use Kruskal-Wallis Rank Sum Test:
kruskal.test(richness2$Simpson ~ sample_data(physeq1)$Age)
# We can also get a list with the p-values resulted of the Wilcoxon Tests considering each pair of groups:
pairwise.wilcox.test(richness2$Simpson, sample_data(physeq1)$Age, p.adj = "bonf")
...
plot_richness(physeq1, x="Age", measures="ACE", color = "Age")+
  geom_boxplot(alpha=0.6)+ 
  theme(legend.position="none", axis.text.x=element_text(angle=45,hjust=1,vjust=1,size=12))
...
# If it is normally distributed, we can run anova tests and check if the variable Age impacts the ACE index:
anova.sh = aov(richness3$ACE ~ sample_data(physeq1)$Age)
summary(anova.sh)
TukeyHSD(anova.sh)
...
# For Non-normally distributed data, we can use Kruskal-Wallis Rank Sum Test:
kruskal.test(richness3$ACE ~ sample_data(physeq1)$Age)
# We can also get a list with the p-values resulted of the Wilcoxon Tests considering each pair of groups:
pairwise.wilcox.test(richness3$ACE, sample_data(physeq1)$Age, p.adj = "bonf")
...
plot_richness(physeq1, x="Age", measures="Observed", color = "Age")+
  geom_boxplot(alpha=0.6)+ 
  theme(legend.position="none", axis.text.x=element_text(angle=45,hjust=1,vjust=1,size=12))
...
# If it is normally distributed, we can run anova tests and check if the variable Age impacts the Observed index:
anova.sh = aov(richness4$Observed ~ sample_data(physeq1)$Age)
summary(anova.sh)
TukeyHSD(anova.sh)
...
# For Non-normally distributed data, we can use Kruskal-Wallis Rank Sum Test:
kruskal.test(richness4$Observed ~ sample_data(physeq1)$Age)
# We can also get a list with the p-values resulted of the Wilcoxon Tests considering each pair of groups:
pairwise.wilcox.test(richness4$Observed, sample_data(physeq1)$Age, p.adj = "bonf")
