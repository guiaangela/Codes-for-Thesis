...
anova.sh = aov(richness$Shannon ~ sample_data(physeq1)$Stage)
summary(anova.sh)
TukeyHSD(anova.sh)
...
kruskal.test(richness$Shannon ~ sample_data(physeq1)$Stage)
# We can also get a list with the p-values resulted of the Wilcoxon Tests considering each pair of groups:
pairwise.wilcox.test(richness$Shannon, sample_data(physeq1)$Stage, p.adj = "bonf")
